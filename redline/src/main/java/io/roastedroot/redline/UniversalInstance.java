package io.roastedroot.redline;
...
/**
 * Smart builder that tries Redline native execution first,
 * falling back to Chicory JVM bytecode compilation.
 *
 * <p>Runtime selection:
 * <ol>
 *   <li>If native code is available and a Redline runner is on the classpath
...
public final class UniversalInstance {
    ...
    /**
     * Builder that implements {@link RedlineInstance.Builder} with fallback logic.
     */
    public static final class Builder implements RedlineInstance.Builder {
        ...
        public RedlineInstance build() {
            ...
            if (nativeCode != null) {
                try {
                    var b =
                            RedlineInstance.builder(module)
            ...
            return RedlineInstance.forBytecode(ib.build());
        }
    }
}
