package io.roastedroot.redline.maven;

import io.roastedroot.redline.build.Config;
import io.roastedroot.redline.build.Generator;
import java.io.File;
...
/**
 * Compiles a Wasm module to native code at build time.
...
public class RedlineCompilerMojo extends AbstractMojo {
    ...
    @Parameter(
            required = true,
            defaultValue = "${project.build.directory}/generated-resources/redline-compiler")
    private File targetResourceFolder;

    @Parameter(
            required = true,
            defaultValue = "${project.build.directory}/generated-sources/redline-compiler")
    ...
    public void execute() throws MojoExecutionException {
        getLog().info("Redline: compiling " + name + " from " + wasmFile);
